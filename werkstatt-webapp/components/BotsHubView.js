...
import {
  ...
  CheckCircle2, AlertCircle, ExternalLink, ArrowRight, Link2
} from "lucide-react";
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {isInternal ? (
            ...
            <a
              ...
              className={`inline-flex items-center gap-2 text-white text-sm font-medium px-4 py-2 rounded-lg ${accent.btn}`}
            >
              Säp starten <ArrowRight size={14} />
            </a>
          )}
        </div>
      </div>
    ...
    <div>
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 max-w-2xl">
            Alle Bots teilen denselben Stack: <strong>Supabase</strong> (Daten & Login),{" "}
            <strong>GitHub</strong> (Code & Automatisierung) und <strong>Netlify</strong> (Hosting).
            Säp baut Apps, Checker bewertet Schritte, Zementa berichtet wöchentlich.
          </p>
        </div>
      </div>
      ...
      <section className="mb-8 bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        <ol className="space-y-3 text-sm text-slate-600">
          <li className="flex gap-3">
            ...
            <span><strong>Säp</strong> erzeugt und exportiert Apps über GitHub → Netlify (z. B. Werkstatt-Ideen).</span>
          </li>
          <li className="flex gap-3">
      ...
      <section id="saep-setup" className="bg-slate-900 text-white rounded-xl p-5 shadow-sm">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h2 className="text-sm font-semibold">Säp starten (gleiche Konten)</h2>
        </div>
        <p className="text-sm text-slate-300 mb-3">
          Säp läuft als eigene App im Ordner <code className="text-violet-200">saep/</code> und nutzt dieselben
          Supabase-/GitHub-/Netlify-Konten.
        </p>
        <pre className="text-xs bg-black/40 border border-white/10 rounded-lg p-3 overflow-x-auto text-slate-200 font-mono whitespace-pre-wrap">{`cd saep
cp .env.local.example .env.local   # gleiche SUPABASE_URL + ANON_KEY
npm install
npm run dev`}</pre>
        <p className="text-xs text-slate-400 mt-3">
          SQL: zuerst <code className="text-slate-300">werkstatt-webapp/supabase/schema.sql</code> +{" "}
          <code className="text-slate-300">zementa.sql</code>, dann{" "}
          <code className="text-slate-300">saep/supabase/schema.sql</code> im selben Supabase-Projekt.
        </p>
      </section>
    </div>
  );
